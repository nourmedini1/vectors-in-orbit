...
async def register(user_data: UserRegister):
    ...
    try:
        ...
        try:
            ...
            await requests.post(f"http://{IP_ADDRESS}:8000/events/profile", json=event.model_dump())
...
async def create_review(review: CreateReviewRequest):
    ...
    try:
        ...
        requests.post(f"http://{IP_ADDRESS}:8000/events/review", json=event.model_dump())
...
async def add_text_to_wishlist(request: AddTextToWishlistRequest):
    ...
    try:
        ...
        requests.post(f"http://{IP_ADDRESS}:8000/events/wishlist", json=event.model_dump())
        return item
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
async def add_to_cart(request: AddToCartRequest):
    ...
    try:
        ...
        try:
            ...
            requests.post(f"http://{IP_ADDRESS}:8000/events/cart/add", json=event.model_dump())
...
async def remove_from_cart(request: RemoveFromCartRequest):
    ...
    try:
        ...
        requests.post(f"http://{IP_ADDRESS}:8000/events/cart/remove", json=event.model_dump())
...
async def create_order(request: CreateOrderRequest):
    ...
    try:
        ...
        for item in order.get('items', []):
            ...
            requests.post(f"http://{IP_ADDRESS}:8000/events/purchase", json=event.model_dump())
...
async def review_submit_event(
    ...
    try:
        ...
        await requests.post(f"http://{IP_ADDRESS}:8000/events/review", json=event.model_dump())
...
async def return_purchase(purchase_id: str, request: ReturnPurchaseRequest):
    ...
    try:
        ...
        requests.post(f"http://{IP_ADDRESS}:8000/events/return", json=event.model_dump())
